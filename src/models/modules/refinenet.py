...
class CondCRPBlock(nn.Module):
    def __init__(self, features, n_stages, num_classes):
        super().__init__()
...
class CondRCUBlock(nn.Module):
    def __init__(self, features, n_blocks, n_stages, num_classes):
        super().__init__()
...
class CondRefineBlock(nn.Module):
    def __init__(self, in_planes, features, num_classes, act=nn.ReLU(), start=False, end=False):
        super().__init__()
        ...
        for i in range(n_blocks):
            self.adapt_convs.append(
                CondRCUBlock(in_planes[i], 2, 2, num_classes, act)
            )

        self.output_convs = CondRCUBlock(features, 3 if end else 1, 2, num_classes)

        if not start:
            self.msf = CondMSFBlock(in_planes, features, num_classes)

        self.crp = CondCRPBlock(features, 2, num_classes)
...
class ConditionalResidualBlock(nn.Module):
    def __init__(self, input_dim, output_dim, num_classes, resample=None, adjust_padding=False, dilation=None):
        super().__init__()
    ...
    def forward(self, x, y):
        output = self.elu(self.normalize1(x, y))
        output = self.conv1(output)
        output = self.elu(self.normalize2(output, y))
...
class RefineNet(nn.Module):
    def __init__(self, in_channel, ngf: int = 128, num_classes: int = 10):
        ...
        self.block2 = nn.Sequential(
            ...
            ConditionalResidualBlock(2 * ngf, 2 * ngf, num_classes, resample=None)
        )

        self.block3 = nn.Sequential(
            ConditionalResidualBlock(2 * ngf, 2 * ngf, num_classes, resample='down', dilation=2),
            ConditionalResidualBlock(2 * ngf, 2 * ngf, num_classes, resample=None, dilation=2)
        )

        self.block4 = nn.Sequential(
            ConditionalResidualBlock(2 * ngf, 2 * ngf, num_classes, resample='down',
                                     adjust_padding=False, dilation=4),
            ConditionalResidualBlock(2 * ngf, 2 * ngf, num_classes, resample=None, dilation=4)
        )

        self.refine1 = CondRefineBlock([2 * ngf], 2 * ngf, num_classes, self.norm, start=True)
        self.refine2 = CondRefineBlock([2 * ngf, 2 * ngf], 2 * ngf, num_classes, self.norm)
        self.refine3 = CondRefineBlock([2 * ngf, 2 * ngf], ngf, num_classes, self.norm)
        self.refine4 = CondRefineBlock([ngf, ngf], ngf, num_classes, self.norm, end=True)
    ...
    def forward(self, x, y):
        ...
        layer1 = self.block1(output, y)
        layer2 = self.block2(layer1, y)
        layer3 = self.block3(layer2, y)
        layer4 = self.block4(layer3, y)
